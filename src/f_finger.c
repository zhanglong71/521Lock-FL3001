...
/*******************************************************************************
 * 删除全部指纹
 * 从表现上看，“删除全部指纹及指纹的全部索引”与“删除第一管理者管辖的全部指纹及全部索引”效果相同 !!!是这样吗???
 *******************************************************************************/
...
//	charData_t charData;
	...
	case CMSG_INIT:		/** 开始录入指纹 **/
		...
		g_uiAddUserId = FP_AddByManager(g_uiUserId, ucRgstCnt);
		//if(g_ucAddUserId == 0)								   
		if(g_uiAddUserId == 0)
		{
			msg.msgType = CMSG_INIT;
	...
	case CFIGER_FGPRPRD:
		...
		FP_AddByManager(g_uiUserId, ucRgstCnt);
		
		#if	0
		//if(ucRet != g_ucAddUserId)					   
		if(ucRet != g_uiAddUserId)
		{
			//g_ucAddUserId = ucRet;	 
			g_uiAddUserId = ucRet;
			SetTimer_irq(&g_timer[0], TIMER_100MS, CMSG_INIT);	/** 初始化定时器 **/
	...
	case CMSG_FGRGST:
		if(ucRgstCnt <= 1)
		{
			...
			ucRgstCnt++;
			//ucRet = FP_AddByManager(g_ucUserId, ucRgstCnt);
			ucRet = FP_AddByManager(g_uiUserId, ucRgstCnt);
			if(ucRet != g_ucAddUserId)
			{
				g_ucAddUserId = ucRet;
				SetTimer_irq(&g_timer[0], TIMER_100MS, CMSG_INIT);	/** 初始化定时器 **/
		...
		else if(ucRgstCnt == 2)
		{
			...
			fingerTab_setEntry(g_uiAddUserId, 0xa5);
			flashWrite(g_flash.arrInt);
...
//	unsigned char ucArr[12];
	...
	case CFIGER_DEL:
		...
		FP_DelByManager(g_uiUserId);
		//fingerTab_fill(g_ucUserId, 0xff);
		fingerTab_fill(g_uiUserId, 0xff);
		flashWrite(g_flash.arrInt);	
