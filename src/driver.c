...
//    unsigned char ucResp;
	...
	case CMSG_COMRX:	/** 处理串口数据 **/
		...
		if(ucRet != 0)			/** 收到有效数据 **/
		{
			...
			ucRet = FP_RespChk(ucArr);		/** data valid or not **/
			if((ucRet  == CMD_ACK_XOR_ERR) || (ucRet  == CMD_ACK_SUM_ERR))
			{
				/** 指纹操作数据-失败 **/
				msg.msgType = CMSG_FGOPFAIL;
				inq_irq(&g_msgq, &msg);
			}
			else if(ucRet == CMD_ACK_OK)
			{
				iResult = FP_RespGetResult(ucArr);
				if(iResult != (int)CMD_RESULT_OK)
				{
					if(iResult != (int)CMD_EROLL_AGAIN)		/** 重复注册不算失败 **/
					{
						msg.msgType = CMSG_FGOPFAIL;
						inq_irq(&g_msgq, &msg);
						break;
				...
				case FP_ENROLL:
						/** 注册指纹-正常 **/
					ucRet = FP_RespGetLen(ucArr);
					if(ucRet == 2)
					{
						g_uiAddUserId = FP_RespGetId(ucArr);
					}
					else
					{
						g_uiAddUserId = CINVALIDID;
					}
				...
				case FP_MATCH:
						/** 查找指纹-成功 **/
					//g_ucUserId = FP_RespGetId(ucArr); 
					g_uiUserId = FP_RespGetId(ucArr);
						
					//msg.msgValue = g_ucUserId;
					msg.msgValue = g_uiUserId;
